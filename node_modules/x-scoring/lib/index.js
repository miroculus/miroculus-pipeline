...
function run(cb) {
  ...
  */
  ...
  function processMessage(message, cb) {
    ...
    if (msgObject.requestType == constants.queues.action.LAST_ITEM_TO_SCORE) {
    ...
    else if (msgObject.requestType == constants.queues.action.RESCORE) {
      ...
      function rowHandler(row) {
        ...
        return queueIn.sendMessage(message, function (err) {
          if (err) {
            console.error('failed to queue rescoring item', message);
            return cb(err);
          }
        });
      }
    }
    else if (msgObject.requestType !== constants.queues.action.SCORE) {
