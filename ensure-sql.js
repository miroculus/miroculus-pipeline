...
var cmd = 'sqlcmd -U %DB_USER% -S %DB_SERVER% -P %DB_PASSWORD% -d master -Q "SELECT name FROM sys.databases WHERE name=\'%DB_NAME%1\'" -h -1';
...
child_process.exec(cmd, null, function (error, stdout, stderr) {
  ...
  createdb_command += 'sqlcmd -U %DB_USER% -S %DB_SERVER% -P %DB_PASSWORD% -d master -Q "CREATE DATABASE %DB_NAME%" && ';
