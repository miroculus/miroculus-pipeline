...
var configSql = require('./config/index').sql;
...
function connect(cb) {
    ...
    connection.on('connect', function(err) {
        ...
        console.log('connection established', !(<any>connection).closed);
...
function executeStatement1(cb) {
    connect(function (err, connection) {
        ...
        request.addParameter('Name', TYPES.NVarChar, 'tshello! ' + new Date().getTime());
        /*
    ...
    connect(function(err, connection){
        ...
        try
        {
            ...
            var request = new tedious.Request('UpsertJob', function(err) {
                if (err && (<any>err).number == DBErrors.duplicate) return logError(new Error('video already assigned to user'), cb);
                if (err) return logError(err, cb);
