...
function run(callback) {
    ...
    log.init({
        ...
        //  2.3) parse paper into sentences
        //  2.4) insert each sentence into sentences queue
        ...
        queueIn.init(function (error) {
            if (error) return utils.callbackError(error, callback);
        ...
        function getTriggerMessage(cb) {
            return queueIn.getSingleMessage(function (error, message) {
                if (error) return utils.callbackError(new Error('problem querying trigger queue', error), cb);
                ...
                if (messageDetails && messageDetails.requestType == 'trigger') {
                    ...
                    return queryNewDocumentIDs(fromDate, toDate, function (err) {
                        ...
                        if (error) return utils.callbackError(new Error('error while processing trigger message', error), cb);
                ...
                function deleteQueueMessage(message, cb) {
                    ...
                    return queueIn.deleteMessage(message, function (error) {
                        ...
                        if (error) return utils.callbackError(new Error('error while deleting trigger message', error), cb);
        ...
        function queryNewDocumentIDs(fromDate, toDate, cb) {

            // Checking that a message returned from the queue
            // if no message was returned, the queue is empty
            var toDate = (toDate && moment(toDate)) || moment();
            var fromDate = (fromDate && moment(fromDate)) || moment().add(-3, 'days'); // TODO: change to 0 days (only today)
            ...
            return service.getPapers(fromDate.toDate(), toDate.toDate(), function (error, documents) {
                if (error) return utils.callbackError(new Error('There were several errors while retrieving the papers.', error), cb);
                ...
                // Queue all new document ids (1000 active queuing requests at a time)
                async.eachLimit(documents, 1000, enqueueDocument, function (error) {
                    if (error) return utils.callbackError(new Error('failed to queue messages for documents.', error), cb);
                ...
                function enqueueDocument(doc, cb) {
                    ...
                    return queueOut.sendMessage(message, function (error) {
                        if (error) return utils.callbackError(new Error('There was an error queuing a document.', error), cb);
