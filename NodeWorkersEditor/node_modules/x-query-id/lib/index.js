    ...
    log.init({
        ...
        function getTriggerMessage(cb) {
            return queueIn.getSingleMessage(function (error, message) {
                ...
                try {
                    ...
                    console.error('error while parsing trigger message, deleting message', message, error);
                    return deleteQueueMessage(message, cb);
                }
                
                console.info('found trigger message: ', messageDetails);
                if (messageDetails.requestType === 'trigger') {
                  var data = messageDetails.data || {};
                  ...
                  console.error('message type is not expected, deleting', messageDetails);
                }
        ...
        function queryNewDocumentIDs(data, cb) {
            ...
            return service.getPapers(fromDate.toDate(), toDate.toDate(), function (error, documents) {
                ...
                // TODO: remove
                //documents = documents.splice(0, 100);
                ...
                function enqueueDocument(doc, cb) {
                    ...
                    return queueOut.sendMessage(message, function (err) {
                        if (err) {
                            console.error('There was an error queuing a document', err);
                            return cb(err);
                        }
