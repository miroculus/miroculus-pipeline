...
function run(fileName, callback) {
  ...
  function checkBlob(cb) {
    ...
    blobIn.getFile(fileName, function (err, fileContent) {
      if (err) {
        console.error(err);
        return callback(err);
      }
  ...
  function processSentences(fileContent, cb) {
    ...
    async.eachLimit(lines, 100, queueLine, function (err) {
      if (err) {
        console.error('There was a problem queuing line %s in the file', i);
        return cb(err);
      }
    ...
    function queueLine(line, cb) {
      ...
      var outMessage = parserUtils.createQueueSentenceMessage(constants.sources.GENERAL, docId, index, line);
      
      queueOut.sendMessage(outMessage, function (err) {
        if (err) {
