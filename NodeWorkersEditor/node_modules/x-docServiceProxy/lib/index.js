...
function getDocumentSentences(docId, source, cb) {
    ...
    var urlRequest = config.services.docServiceUrl + '/' + source + '/' + docId;
    ...
    request(urlRequest, function (err, resp, body) {
        ...
        if (!Array.isArray(sentencesArray)) return cb(new Error('Returned JSON is not an array'));
        
        return callback(null, sentencesArray);
    });
}
...
function getPapers(dateFrom, dateTo, callback) {
    ...
    function runSearchRequest(database, startIndex, callback) {
        ...
        service.searchRequest(database, [pdaTimeSpan], MAX_RESULTS, startIndex, service.etypes.edat, -1, function (err, res, cache) {
            ...
            console.info('results return from db {} on dates {}', cache.database, pdaTimeSpan);
            ...
            console.info('db {} on dates {} with result count {}', cache.database, pdaTimeSpan, documents.length);
    ...
    return async.parallel([
        ...
        if (err) {
            console.error('Completed retreiving db new ids. There was a problem scanning the {}\nError:\n{}', pdaTimeSpan, err);
            return callback(err);
        }
        
        console.info('Completed retrieving db new ids on date span {}. Filtering only new ids...', pdaTimeSpan);
        ...
        return checkDocuments(allDocuments.documents, function (err, documents) {
            ...
            console.info('filtered {} documents', documents.length);
